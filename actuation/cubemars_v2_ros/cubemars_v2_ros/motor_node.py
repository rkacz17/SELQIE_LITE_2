...
class MotorNode(Node):
    ...
    def __init__(self):
        ...
        )  # Seconds before stale commands are replaced with neutral output
    ...
    def _tick_control(self):
        ...
        with self._lock:
            ...
            # A stale cached velocity/torque command can make a motor run away if
            # the upstream publisher dies, so fail safe to neutral output.
            stale = (
                self.command_timeout > 0.0
                and time.monotonic() - self._last_cmd_time > self.command_timeout
            )
            if stale and not self._neutral_hold:
                if not self._stale_command_warned:
                    self.get_logger().warn(
                        f"No fresh command for {self.command_timeout:.3f}s; sending neutral output to {self.joint_name}"
                    )
